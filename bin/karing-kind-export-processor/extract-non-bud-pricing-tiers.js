...
(async function() {
    ...
    let tierCount = pricingTiers.length + 1;
    ...
    for(let label of Object.keys(labelWholesaleMap)) {
        ...
        if(row.some(_package => extractWeightInGrams(_package))) {
            ...
            for(let variationSize of row) {
                ...
                let price = variationSize['Price'].replace(/\$/g, '');

                weightTiers[variationSize.weight] = price;
            ...
            if(existingTier) {
                ...
                console.log(`Found existing tier ${JSON.stringify(existingTier)}`);
                await product.save();
            }
            else {
